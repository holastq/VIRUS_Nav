...
void ExplorationFSM::init(ros::NodeHandle& nh)
{
  ...
  poisoned_goal_region_sub_ = nh.subscribe(
      "/adversarial_eval/poisoned_goal_region", 10,
      &ExplorationFSM::poisonedGoalRegionCallback, this);
  trap_lock_sub_ = nh.subscribe(
      "/adversarial_eval/trap_lock", 10, &ExplorationFSM::poisonedGoalRegionCallback,
      this);

  poisoned_goal_region_active_ = false;
  have_poisoned_goal_region_target_ = false;
  poisoned_goal_region_anchor_.setZero();
  poisoned_goal_region_target_.setZero();
  poisoned_goal_region_radius_ = 1.0;
  poisoned_goal_region_orbit_index_ = 0;
...
int ExplorationFSM::callActionPlanner()
{
  ...
  // Keep the original collision recovery before the poisoned-region hook. When
  // Habitat draws a red border, the previous action has collided; if the
  // security-eval hook returns first, the normal escape routine never runs and
  // the video looks frozen against an obstacle.
  if (tryPlanPoisonedGoalRegionAction(current_pos, current_yaw)) {
...
bool ExplorationFSM::tryPlanPoisonedGoalRegionAction(
    Vector2d current_pos, double current_yaw)
{
  if (!poisoned_goal_region_active_)
    return false;

  constexpr int kOrbitCandidateNum = 12;
  const double target_reached_distance = 0.35;
  const double radius = std::max(0.35, poisoned_goal_region_radius_);

  if (have_poisoned_goal_region_target_ &&
      (current_pos - poisoned_goal_region_target_).norm() < target_reached_distance) {
    poisoned_goal_region_orbit_index_ =
        (poisoned_goal_region_orbit_index_ + 3) % kOrbitCandidateNum;
    have_poisoned_goal_region_target_ = false;
  }

  for (int offset = 0; offset < kOrbitCandidateNum; ++offset) {
    int idx = (poisoned_goal_region_orbit_index_ + offset) % kOrbitCandidateNum;
    double theta = 2.0 * M_PI * double(idx) / double(kOrbitCandidateNum);
    Vector2d candidate = poisoned_goal_region_anchor_ +
                         radius * Vector2d(std::cos(theta), std::sin(theta));

    expl_manager_->path_finder_->reset();
    if (expl_manager_->path_finder_->astarSearch(current_pos, candidate, 0.25, 0.02) !=
        Astar2D::REACH_END) {
      continue;
    }

    vector<Vector2d> path = expl_manager_->path_finder_->getPath();
    if (path.empty())
      continue;

    auto ed = expl_manager_->ed_;
    ed->next_pos_ = candidate;
    ed->next_best_path_ = path;
    poisoned_goal_region_target_ = candidate;
    poisoned_goal_region_orbit_index_ = idx;
    have_poisoned_goal_region_target_ = true;
    fd_->newest_action_ = planNextBestAction(current_pos, current_yaw, path, true);

    ROS_WARN_THROTTLE(1.0,
        "[SecurityEval] Poisoned goal region active. Belief center=(%.2f, %.2f), search waypoint=(%.2f, %.2f), action=%d",
        poisoned_goal_region_anchor_(0), poisoned_goal_region_anchor_(1),
        candidate(0), candidate(1), fd_->newest_action_);
    return true;
  }

  ROS_WARN_THROTTLE(1.0,
      "[SecurityEval] Poisoned goal region is active, but no reachable search waypoint was found.");
  return false;
}
...
void ExplorationFSM::poisonedGoalRegionCallback(
    const std_msgs::Float32MultiArrayConstPtr& msg)
{
  if (msg->data.size() < 4 || msg->data[0] < 0.5) {
    poisoned_goal_region_active_ = false;
    have_poisoned_goal_region_target_ = false;
    return;
  }

  Vector2d new_anchor(msg->data[1], msg->data[2]);
  if (!poisoned_goal_region_active_ ||
      (new_anchor - poisoned_goal_region_anchor_).norm() > 0.15) {
    have_poisoned_goal_region_target_ = false;
    poisoned_goal_region_orbit_index_ = 0;
  }

  poisoned_goal_region_active_ = true;
  poisoned_goal_region_anchor_ = new_anchor;
  poisoned_goal_region_radius_ = std::max(0.35, double(msg->data[3]));
}
