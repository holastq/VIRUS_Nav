...
public:
  ...
  HalfEdgeMesh(
      const MeshBuilder<FloatType>& builderObject, const VertexDataSource<FloatType>& vertexData)
  {
    ...
    for (const auto& halfEdge : builderObject.m_halfEdges) {
      if (!halfEdge.isDisabled()) {
        m_halfEdges.push_back(
            { static_cast<IndexType>(halfEdge.m_endVertex), static_cast<IndexType>(halfEdge.m_opp),
                static_cast<IndexType>(halfEdge.m_face), static_cast<IndexType>(halfEdge.m_next) });
...
public:
  ...
  ConvexHull(const MeshBuilder<T>& mesh, const VertexDataSource<T>& pointCloud, bool CCW,
  ...
  inline void writeWaveformOBJ(
      const std::string& filename, const std::string& objectName = "quickhull") const
  {
...
class QuickHull {
  ...
  inline void setupInitialTetrahedron()
  {
    ...
    for (size_t i = 0; i < 6; i++) {
      for (size_t j = i + 1; j < 6; j++) {
        const T d =
            m_vertexData[m_extremeValues[i]].getSquaredDistanceTo(m_vertexData[m_extremeValues[j]]);
        if (d > maxD) {
          maxD = d;
    ...
    if (maxD == m_epsilonSquared) {
      ...
      return m_mesh.setup(0, std::min((size_t)1, vertexCount - 1),
          std::min((size_t)2, vertexCount - 1), std::min((size_t)3, vertexCount - 1));
    }
    ...
    if (maxD == m_epsilonSquared) {
      ...
      for (; it != m_vertexData.end() && !(*it != m_vertexData[selectedPoints.first] &&
      ...
      for (it = m_vertexData.begin();
           ...
           !(*it != m_vertexData[selectedPoints.first] &&
               *it != m_vertexData[selectedPoints.second] && *it != m_vertexData[thirdPoint]);
           it++) {
      }
    ...
    if (maxD == m_epsilon) {
      ...
      m_planar = true;
      const vec3 N1 = mathutils::getTriangleNormal(
          baseTriangleVertices[1], baseTriangleVertices[2], baseTriangleVertices[0]);
      m_planarPointCloudTemp.clear();
      m_planarPointCloudTemp.insert(
          m_planarPointCloudTemp.begin(), m_vertexData.begin(), m_vertexData.end());
    ...
    for (size_t i = 0; i < horizonEdgeCount - 1; i++) {
      ...
      for (size_t j = i + 1; j < horizonEdgeCount; j++) {
        const size_t beginVertex =
            m_mesh.m_halfEdges[m_mesh.m_halfEdges[horizonEdges[j]].m_opp].m_endVertex;
  ...
  // Find indices of extreme values (max x, min x, max y, min y, max z, min z) for the given point
  // cloud
  ...
  inline void createConvexHalfEdgeMesh()
  {
    ...
    while (!m_faceList.empty()) {
      ...
      if (!reorderHorizonEdges(m_horizonEdges)) {
        ...
        auto it = std::find(
            tf.m_pointsOnPositiveSide->begin(), tf.m_pointsOnPositiveSide->end(), activePointIndex);
  ...
  inline void buildMesh(
      const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices, T eps)
  {
  ...
  inline ConvexHull<T> getConvexHull(
      const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices, T eps)
  {
...
public:
  ...
  inline ConvexHull<T> getConvexHull(const std::vector<Vector3<T>>& pointCloud, bool CCW,
  ...
  inline ConvexHull<T> getConvexHull(const T* vertexData, size_t vertexCount, bool CCW,
      bool useOriginalIndices, T eps = defaultEps<T>())
  {
  ...
  inline HalfEdgeMesh<T, size_t> getConvexHullAsMesh(
      const T* vertexData, size_t vertexCount, bool CCW, T eps = defaultEps<T>())
  {
    VertexDataSource<T> vertexDataSource((const vec3*)vertexData, vertexCount);
