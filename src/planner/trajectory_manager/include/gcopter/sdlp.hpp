...
inline void lp_min_lin_rat(const bool degen, const double cw_vec[2], const double ccw_vec[2],
    const double n_vec[2], const double d_vec[2], double opt[2])
{
...
inline int wedge(const double (*halves)[2], const int m, int* next, int* prev, double cw_vec[2],
    double ccw_vec[2], bool* degen)
{
  int i;
  double d_cw, d_ccw;
  bool offensive;
...
template <int d>
inline void vector_up(
    const double* equation, const int ivar, const double* low_vector, double* vector)
{
  vector[ivar] = 0.0;
  for (int i = 0; i <= d; i++) {
    if (i != ivar) {
...
template <int d>
inline void vector_down(
    const double* elim_eqn, const int ivar, const double* old_vec, double* new_vec)
{
  double ve = 0.0;
  double ee = 0.0;
  for (int i = 0; i <= d; i++) {
...
template <int d>
inline void plane_down(
    const double* elim_eqn, const int ivar, const double* old_plane, double* new_plane)
{
...
*/
{
  ...
  for (int i = 0; i != m; i = next[i]) {
    ...
    if (val < -(d + 1) * eps) {
      ...
      /* solve sub problem */
      status = linfracprog<d - 1>(
          new_halves, max_size, i, new_n_vec, new_d_vec, new_opt, new_work, next, prev);
      /* back substitution */
      if (status != INFEASIBLE) {
...
inline int linfracprog<1>(const double* halves, const int max_size, const int m,
    const double* n_vec, const double* d_vec, double* opt, double* work, int* next, int* prev)
{
  if (m > 0) {
...
*/
{
  ...
  int status = sdlp::linfracprog<d>(halves.data(), m, m, n_vec.data(), d_vec.data(), opt.data(),
      work.data(), next.data(), prev.data());
