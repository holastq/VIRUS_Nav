...
struct lbfgs_parameter_t {
   ...
   */
  /* 近似逆 hessian 矩阵的校正次数。*/
  /* 存储先前m次迭代的计算结果，以逼近当前迭代的逆 hessian
   * 矩阵。此参数控制有限内存的大小*（更正）。默认值为 8。* 不推荐小于 3
   * 的值。较大的值会导致计算时间过长。*/
  int mem_size = 8;
   ...
   */
  /* 用于收敛测试的 Epsilon。*/
  /* 不要在不光滑的情况下使用它！ 将其设置为 0.0 并对非平滑函数使用基于过去增量的测试。*/
  /* 此参数确定要找到的解决方案的精度。当||g(x)||_inf / max(1, ||x||_inf) < g_epsilon, *
   * 时，最小化终止，其中 ||.||_inf 是无穷范数。默认值为 1.0e-5。*/
   ...
   */
  /* * 基于增量的收敛测试的距离。*/
  /* 此参数确定迭代中的距离，以计算成本函数的下降率。如果此参数的值为零，则库不执行基于 delta
   * 的收敛测试。默认值为 3。*/
  int past = 3;
   ...
   */
  // Delta 用于收敛测试。
  // 此参数确定成本函数的最小下降率。当满足以下条件时，库将停止迭代：|f' - f| /max(1, |f|) <
  // delta，其中 f' 是过去迭代之前的成本值，f 是 * 当前迭代的成本值。默认值为 1.0e-6。
  double delta = 1.0e-6;
   ...
   */
  // 最大迭代次数。
  // 当迭代计数超过此参数时，lbfgs_optimize() 函数会以 * ::LBFGSERR_MAXIMUMITERATION
  // 状态代码终止最小化过程。将此参数设置为零会继续最小化过程，直到收敛或错误。默认值为 0。
   ...
   */
  /** 线搜索的最大试验次数。
   * 此参数控制线搜索例程的每次迭代的函数和梯度评估次数。默认值为 64。
   */
   ...
   */
  /** * 行搜索例程的最小步骤。
   * 默认值为 1.0e-20。这个值不需要修改，除非指数对于正在使用的机器来说太大，或者问题被严重缩放（在这种情况下，指数应该增加）
   */
  ...
  /** * 行搜索的最大步长。
   * 默认值为 1.0e+20。
   * 这个值不需要修改，除非指数对于正在使用的机器来说太大，或者除非问题被严重缩放（在这种情况下，指数应该增加）。
   */
  ...
  /** * 控制线搜索例程准确性的参数。
   * 默认值为 0.9。
   * 如果函数和梯度评估相对于迭代的成本是廉价(inexpensive)的（解决非常大的问题时有时会出现这种情况），则将此参数设置为较小的值可能是有利的。
   * 典型的小值是 0.1。此参数应大于 f_dec_coeff 参数且小于 1.0。
   */
   ...
   * 该参数对 L-BFGS 执行所谓的谨慎更新，尤其是在收敛不充分时。
   * 该参数必须为正数，但实际上最好小于 1.0e-3。
   */
   ...
   */
  /** * 浮点值的机器精度。默认值为 1.0e-16。
   * 此参数必须是客户端程序设置的正值，以估计机器精度。
   */
...
inline const char* lbfgs_strerror(const int err)
{
  switch (err) {
    ...
    case LBFGSERR_MAXIMUMLINESEARCH:
      return "Line search reaches the maximum try number, assumptions not satisfied or precision "
             "not achievable.";
